...
async function loadSettings() {
  try {
    ...
    removeIrrelevantElements =
      result.removeIrrelevantElements === true || result.removeIrrelevantElements === false
        ? result.removeIrrelevantElements
        : false;
    blockClickbaitVaguetitles =
      result.blockClickbaitVaguetitles === true || result.blockClickbaitVaguetitles === false
        ? result.blockClickbaitVaguetitles
        : false;
...
chrome.storage.onChanged.addListener((changes, namespace) => {
  ...
  if (namespace === "sync" && changes.removeShorts) {
    removeShorts =
      changes.removeShorts.newValue === true || changes.removeShorts.newValue === false
        ? changes.removeShorts.newValue
        : false;
    logDebug("YouTube Video Blocker: Remove Shorts changed to:", removeShorts);
  }
  if (namespace === "sync" && changes.removeIrrelevantElements) {
    removeIrrelevantElements =
      changes.removeIrrelevantElements.newValue === true || changes.removeIrrelevantElements.newValue === false
        ? changes.removeIrrelevantElements.newValue
        : false;
  ...
  if (namespace === "sync" && changes.blockClickbaitVaguetitles) {
    blockClickbaitVaguetitles =
      changes.blockClickbaitVaguetitles.newValue === true || changes.blockClickbaitVaguetitles.newValue === false
        ? changes.blockClickbaitVaguetitles.newValue
        : false;
    ...
    chrome.contextMenus.create(
      {
        ...
        targetUrlPatterns: ["*://*.youtube.com/watch?v=*", "*://*.youtube.com/shorts/*"],
      },
      () => {
        if (chrome.runtime.lastError) {
          console.error(
            "YouTube Video Blocker: Error creating channel block from video menu:",
