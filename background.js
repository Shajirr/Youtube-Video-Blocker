...
async function loadSettings() {
  try {
    ...
    removeShorts = result.removeShorts ?? false;
    removeIrrelevantElements = result.removeIrrelevantElements ?? false;
    blockClickbaitVaguetitles = result.blockClickbaitVaguetitles ?? false;
...
browser.storage.onChanged.addListener((changes, namespace) => {
  ...
  if (namespace === "sync") {
    if (changes.removeShorts) {
      removeShorts = changes.removeShorts.newValue ?? false;
      logDebug("YouTube Video Blocker: Remove Shorts changed to:", removeShorts);
    }
    if (changes.removeIrrelevantElements) {
      removeIrrelevantElements = changes.removeIrrelevantElements.newValue ?? false;
      logDebug("YouTube Video Blocker: Remove irrelevant elements changed to:", removeIrrelevantElements);
    }
    if (changes.blockClickbaitVaguetitles) {
      blockClickbaitVaguetitles = changes.blockClickbaitVaguetitles.newValue ?? false;
      logDebug("YouTube Video Blocker: Block clickbait / vague titles changed to:", blockClickbaitVaguetitles);
    }
  }
});

// Target patterns for context menu items
const YT_ANY = ["*://*.youtube.com/*"];
const YT_VIDEOS = ["*://*.youtube.com/watch?v=*", "*://*.youtube.com/shorts/*"];
const YT_CHANNELS = ["*://*.youtube.com/@*"];

// Define context menu items with appropriate contexts and URL patterns
const MENU_ITEMS = [
  {
    id: "block-youtube-video",
    title: "\u200BBlock Video",
    contexts: ["link"],
    documentUrlPatterns: YT_ANY,
    targetUrlPatterns: YT_VIDEOS,
  },
  {
  ...
  },
  {
    ...
    documentUrlPatterns: YT_CHANNELS,
  },
  {
...
async function setupContextMenus() {
  ...
  const results = await Promise.allSettled(MENU_ITEMS.map((item) => browser.contextMenus.create(item)));

  results.forEach((result, i) => {
    if (result.status === "rejected") {
      console.error(`YouTube Video Blocker: Error creating menu "${MENU_ITEMS[i].id}":`, result.reason);
    }
  });
    ...
    browser.tabs.sendMessage(
      ...
      (response) => {
        if (browser.runtime.lastError) {
          console.error("Error sending blockVideo message:", browser.runtime.lastError);
      ...
      (response) => {
        if (response && response.channelName) {
          browser.tabs.sendMessage(
            ...
            (blockResponse) => {
              if (browser.runtime.lastError) {
                console.error("Error blocking channel:", browser.runtime.lastError);
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  logDebug("Background received message:", message);

  if (message.action === "analyzeTitle") {
    const result = analyzeYTTitle(message.title);
    logDebug("Background sending result:", result);
    sendResponse(result);
...
function analyzeYTTitle(title) {
  ...
  const isThisExpletive = doc.match("^this is (how|why|what|where)").found;
  ...
  const clickbaitPhrases = [
    ...
    // Vague "is [dramatic adjective]" without a real subject
    /\b(is|got|just\s+got|became)\s+(crazy|insane|wild|unreal|unbelievable|mind-?blowing|shocking|broken|ruined|over|done|finished|dead)\b/i,

    // Curiosity gap: "Here's why/what/how"
    /here'?s (why|what|how)/i,

    // Vague comparative: "makes it/this safer/better/etc."
    /makes (it|this|that) \w+(er|est)\b/i,

    // Scarcity / exclusivity bait: "only the X", "only one", "only 2.5%", "the only X"
    /only (the |one |[0-9])/i,
    /the only\b/i,

    // "I was not expecting this/it"
    /not expecting (this|it|that)/i,

    // Vague imperative with deictic: "fix this", "try this", "use this", etc.
    /^(fix|try|use|avoid|do|stop|get|see|watch|check) (this|that|it)\b/i,

    // Vague "they/he/she" accusatory question framing
    /^why (do |did |are |were |won'?t )?(they|he|she)\b/i,
  ];
  ...
  if (deictics.found) {
    ...
    const isTimeReference = doc.match(
      "(this|that) (year|month|week|day|time|morning|evening|night|decade|century)",
    ).found;
    ...
    const isPresentational =
      properNounAfterThis &&
      (hasMultipleWords || doc.match("^this is #Place").found || doc.match("^this is #Person").found);

    // E. Check for vague "This/That is [Adjective]" without a clear subject
    const isVagueAdjective =
      doc.match("^(this|that) is (the)? (#Adverb|#Adjective)+").found && !isThisExpletive && !isPresentational;
  ...
  if (titleStartsWithVague) {
    ...
    const isThisPresentational =
      isThisStart && (doc.match("^this is #Place").found || doc.match("^this is #Person").found);
  ...
  // Special Case: "This is [Place/Person]" is a valid presentational anchor
  const isPresentational = doc.match("^this is (#Place|#Person)").found;

  // "Empty Nouns" that don't count as anchors
  const emptyNouns =
    "(#Pronoun|thing|things|something|everything|nothing|it|this|that|these|those|i|we|my|me|you|your|our|so|much)";

  // Real anchors: Nouns that aren't pronouns or vague placeholders
  const concreteNounMatch = doc.match("#Noun").not(emptyNouns);
  ...
  const firstPersonVague =
    isFirstPerson && doc.match("(this|that|it|these|those)").found && !hasConcreteNoun && !hasProperNoun && !hasValue;

  const noAnchor = !hasConcreteNoun && !hasProperNoun && !hasValue && !isFirstPerson && !isPresentational;
